...
Error messages **should** help a reasonably technical user _understand_
and _resolve_ the issue, and **should not** assume that the user is an
...
The `reason` field is a short snake*case description of the cause of the
...
`[A-Z]A-Z0-9*]+[A-Z0-9]`. (This is UPPER_SNAKE_CASE, without leading
...
- If the RPC has _always_ returned `ErrorInfo` with machine-readable
...
_not_ the direct value of `google.rpc.Status.code`. For example, a `google.rpc.Status`
...
_every_ error response that enables clients to use such information
programmatically.
