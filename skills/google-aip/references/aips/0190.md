...
`pubsub.googleapis.com`, the term _interface name_ refers to the name
...
You can think of the _service name_ as a reference to the actual
implementation of a set of APIs, while the _interface name_ refers to
...
In the rare case where an _interface name_ would conflict with another
